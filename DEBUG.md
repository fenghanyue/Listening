...
19. **【2026-07-09 新增，用户反馈】手机切后台，当前歌曲播完后不会自动接下一首，播放器就停住了**：用户的手机环境是 **Android + Edge 浏览器 App**（Chromium 内核，和 Android Chrome 同源）。要求先验证是否真实存在，暂不要求本次修复。核查代码（未做真机复现，见下方"验证局限"）发现三点确凿的代码事实，足以支撑"大概率真实存在"这个结论：① 全文件 grep `mediaSession`/`MediaMetadata` 零命中——完全没有接入 Media Session API，这是 Android Chromium 系浏览器判断"这个后台 tab 正在播放重要媒体、应该被优先保活并给通知栏媒体控制"的标准信号，缺了它，Android 系统的后台管理（Doze 省电、部分厂商 ROM 的激进后台查杀）更容易在息屏/切后台一段时间后冻结或回收这个 tab 的 JS 执行上下文；② 全文件 grep `visibilitychange`/`hidden`/`lifecycle` 也零命中，代码完全没有处理"页面切到后台"这件事；③ 自动切歌链路 `handleTrackEnd()`（[:973](examples/Listening%20Player.dc.html:973) 附近）→ `startTrack()`（[:963](examples/Listening%20Player.dc.html:963) 附近）→ `loadAndPlay()` → `startPlayback()` 里的 `a.play().catch(e => { console.error(...); this.showToast('播放失败'); })`（[:1015](examples/Listening%20Player.dc.html:1015)）——如果 tab 的 JS 上下文已经被冻结，这个 `ended` 回调根本不会执行；就算没冻结、只是新的 `play()` 调用被浏览器拒绝，失败也是完全静默的（`showToast` 在"O. 移除全局 toast 提示"改动后是空函数），且 `playing` 状态在调用 `play()` 之前就已乐观置为 `true`，播放/暂停图标大概率还会继续显示"正在播放"——这和用户描述的"播完一首就静悄悄停住"完全吻合。**验证局限**：以上是代码走查得出的高置信度假设，不是在用户真机上实测复现的——本环境是无头 Chromium + 桌面沙箱，没有真实 Android 系统环境，Android 的后台冻结/查杀策略又和具体机型、ROM、电池优化设置强相关，伪造 `visibilitychange` 事件不能还原真实的系统级冻结行为，所以没有做这个"伪造后台"的实测；以后验证修复效果需要在用户的真机（Android + Edge，切后台/息屏几分钟）上测。**大方向的修复思路**（未实现、未展开设计）：接入 Media Session API（`navigator.mediaSession.metadata` + `setActionHandler`）是这类问题的标准解法，同时能顺带获得系统通知栏/锁屏的媒体控制。
