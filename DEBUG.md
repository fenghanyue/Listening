...
> 提取 SoundCloud API，接入 Listening 聚合音乐搜索。
...
- [x] **歌单 CRUD** — 新建歌单、添加歌曲、删除歌曲、删除歌单，localStorage 持久化 ✅
- [x] **导入歌单** — 从 JSON 文件读取并合并歌单（同名合并去重） ✅
- [x] **导出歌单** — 歌单下载为 JSON 文件（含元数据 + 歌曲信息） ✅
- [x] **显示歌词** — 全屏歌词模式 + 进度联动 + 手动滚动暂停自动滚动 ✅
- [ ] **显示歌曲图片** — 播放器封面大图 + 旋转动画 + 列表缩略图 + 无封面默认占位图
- [ ] **前端 UI 美化** — 进一步优化：移动端适配、交互动效
