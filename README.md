...
跨平台音乐播放器：网易云音乐 / QQ音乐 / SoundCloud 聚合搜索，支持歌单管理、网易云歌单导入、本地持久化和音频/封面离线缓存。前端页面和 CORS 代理合并成一个 Node 进程，可以直接部署到公网。

> ⚠️ 本项目仅供学习交流使用，请勿用于任何商业用途。搜索/播放依赖的均为第三方反向代理接口，非官方 API，不可控、可能随时失效，也没有版权授权。

**在线体验**：[listening-5bnv.onrender.com](https://listening-5bnv.onrender.com)（Render 免费实例，闲置后可能需要几十秒冷启动）

## 功能

- **聚合搜索**：网易云音乐 / QQ音乐 / SoundCloud 并行搜索，按源交错排列结果，可单独勾选/取消某个源
- **播放**：进度条、音量、三种播放模式（列表循环 / 随机 / 单曲循环）、歌词滚动
- **歌单管理**：新建歌单、加入/移出歌单、拖拽调整播放队列顺序
